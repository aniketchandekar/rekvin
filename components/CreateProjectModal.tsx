...
interface CreateProjectModalProps {
  isOpen: boolean;
  onClose: () => void;
  onProjectCreate: (projectData: Omit<Project, 'id' | 'createdAt' | 'resources' | 'workflows'>) => void;
}
...
const CreateProjectModal: React.FC<CreateProjectModalProps> = ({ isOpen, onClose, onProjectCreate }) => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setError('');
    onProjectCreate({ name, goal, type, platform, userRole });
  };

  if (!isOpen) return null;

  const selectedRoleInfo = userRolesWithIcons.find(r => r.role === userRole)!;
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="projectGoal" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Goal of the Project</label>
            <textarea
              id="projectGoal"
              value={goal}
          ...
          <div className="flex justify-between items-center pt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm font-semibold rounded-lg transition-colors text-red-700 dark:text-red-400 border border-red-400 dark:border-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 dark:focus:ring-offset-slate-900"
            >
              Cancel
            </button>
